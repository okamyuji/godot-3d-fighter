...
| `MaxHitSpheresPerWindow` | 4 | 1判定区間の攻撃判定の上限（ADR-0006） |
| `MaxHurtSpheresPerWindow` | 8 | 1判定区間、1姿勢のやられ判定の上限（ADR-0006） |
...
godot-3d-fighter/
  ...
  src/Core/
    ...
    Data/  CharacterData.cs MoveData.cs MoveKind.cs Posture.cs HitHeight.cs ThrowSide.cs HitWindow.cs
           MotionSegment.cs HitSphere.cs StageData.cs Rules.cs
    ...
    Sim/   MatchSimulator.cs ActionSelector.cs Movement.cs BodyPush.cs Facing.cs CameraSide.cs
           HitResolver.cs ThrowResolver.cs WakeUp.cs RoundResolver.cs Xorshift32.cs StateHash.cs
...
すべての型は`namespace Godot3dFighter.Core.*`に置きます。`Math/`、`Input/`、`State/`の構造体は`readonly`または`[StructLayout(LayoutKind.Sequential, Pack = 1)]`で、参照型の項目を持たない形です。`Data/`の型のうち`HitSphere`は`readonly record struct`で、それ以外と`MatchContext`はクラスです。列挙型はすべて基底型を`byte`にします。
...
`Vec3Fix`は`X`、`Y`、`Z`の3つの`Fix16`を持つ`readonly record struct`で、12バイトです。加減算、スカラー倍、`Clamp`、`HorizontalDistanceSquared(Vec3Fix other)`（XとZだけの距離の二乗を`long`で返す）を持ちます。
...
| `StandHurtSpheres` | `HitSphere`の配列（最大8） | 立ちのやられ判定 |
| `CrouchHurtSpheres` | `HitSphere`の配列（最大8） | しゃがみのやられ判定 |
| `DownHurtSpheres` | `HitSphere`の配列（最大8） | ダウンと転がりのやられ判定 |
...
`HitSphere`は`Center`（`Vec3Fix`、キャラ座標）と`Radius`（`Fix16`）です。

### StageDataとRules

`StageData`は`RingRadius`（`Fix16`）と`StartDistance`（`Fix16`、開始時の2人の距離）を持ちます。`data/stages/default.json`の値は、`RingRadius`が6.0、`StartDistance`が2.0です。
...
```json
{
  ...
  "bodyRadius": 0.3,
  "standHurtSpheres": [ { "x": 0, "y": 0.5, "z": 0, "r": 0.35 }, { "x": 0, "y": 1.3, "z": 0, "r": 0.35 } ],
  "crouchHurtSpheres": [ { "x": 0, "y": 0.4, "z": 0, "r": 0.4 } ],
  "downHurtSpheres": [ { "x": 0, "y": 0.15, "z": 0, "r": 0.5 } ],
  "moves": [
    {
      ...
      "windows": [ { "from": 10, "to": 12, "hit": [ { "x": 0.75, "y": 1.3, "z": 0, "r": 0.2 } ], "hurt": [] } ]
    }
  ]
}
```

実数の項目はDTOで`decimal`として受け、`Fix16.FromDecimal`で変換します。列挙型の項目は値の名前の文字列です。
