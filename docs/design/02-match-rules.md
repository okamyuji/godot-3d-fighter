...
7. 体の押し合いと壁 押し合いでは、2人の水平距離が`BodyRadius`の和より短ければ、足りない分を半分ずつ、互いに離す向きへ押し出します。続けて、2人の位置を確定します。確定で一方だけの位置が変わり、まだ水平距離が`BodyRadius`の和より短い時は、足りない分だけもう一方を押し出します。向きは、位置が変わった側から離す向きです。その後、もう一方の位置を確定します。ここまでで2人とも位置の確定で位置が変わっていれば、2人の`Position`を手順6の前の値へ戻します。どちらかが`Throwing`か`Thrown`なら押し合いは行わず、位置の確定だけを行います。
