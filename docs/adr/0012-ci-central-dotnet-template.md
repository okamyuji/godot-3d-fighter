...
主要導線の走破にはCI上でGodot本体が要ります。GitHub Releasesの公式zip（`Godot_v4.7.2-stable_mono_linux_x86_64.zip`）を版とSHA-256で検証して使う方式と、第三者のコンテナイメージを比べました。公式配布物は供給元がgodotengineで、チェックサム検証で改ざんを検出できます。

## 決定

- 中央リポジトリに`.github/workflows/dotnet-ci.yml`を追加します。入力は`dotnet-version`（既定`10.0.x`）、`godot-version`（既定`4.7.2`）、`godot-sha256`（必須）、`run-e2e`（既定true）です。
...
- Godot本体は公式zipを取得し、SHA-256を照合してから展開します。zipはactionsのキャッシュに保存します。

## 影響

- 中央リポジトリの変更は`central-workflow-release`の手順（タグ付け替え）で公開します。
- 本リポジトリからのpushと中央リポジトリの変更には、`gh auth login`の再認証が要ります。
