...
- `ci.yml`の各ジョブは`permissions`で`GITHUB_TOKEN`の権限を明示します。`dotnet-ci.yml`を呼ぶ`ci`ジョブは`contents: read`だけで、security-scanを呼ぶ`security`ジョブは`contents: read`と`pull-requests: write`です。
