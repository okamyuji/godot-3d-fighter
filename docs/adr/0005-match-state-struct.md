...
- 不変条件C-08として「`Unsafe.SizeOf<MatchState>()`が全項目のサイズの和と一致する（詰め物が無い）」を置き、`MatchStateLayoutTests`で確かめます。
- 不変条件C-09として「同じ状態と同じ入力から`Step`は同じ状態と同じハッシュを返す」を置き、`MatchSimulatorTests`で確かめます。
